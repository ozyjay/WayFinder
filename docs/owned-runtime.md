...
The controller has a positive iteration limit, cancellation checks, explicit terminal statuses, validation-failure limits, and a deterministic Fast-to-Deep escalation policy. For the narrow direct-file-read slice, Auto also applies a deterministic evidence-coverage check: a final answer must retain at least 60% of the distinct source terms of sufficient length. A failed Fast answer receives one repair attempt, then escalates to Deep; the generic validation code and escalation are recorded without source terms. A direct filename that is missing or ineligible is handled through the same bounded repair policy: the model receives only a sanitised failure instruction, while the requested path remains excluded from durable state and diagnostics. Diagnostics label this `recoverable-tool-error`. Other filesystem failures remain terminal. Explicit Fast and Deep tasks remain pinned and do not escalate. The default five-iteration limit leaves room for discovery, read, repair, escalation, and final response. It accepts one tool request per inference in this first slice.
...
The sidebar's live debug trace follows the same privacy boundary by default. It is ephemeral UI state rather than persisted telemetry, and shows only iteration numbers, model tiers, tool IDs, validation codes, escalation decisions, sanitised operational errors and any removed backend control-token names. It does not show prompts, paths, tool arguments, file contents, backend details or raw tool results. For local development, `wayfinder.runtime.showToolArgumentsInDebugTrace` can explicitly expose JSON tool arguments in this ephemeral trace, capped at 500 characters. The setting defaults to `false`, may reveal filenames or other sensitive values, and never changes persisted diagnostics.

The ModelDeck client removes a narrow allow-list of known chat-template terminators only when they leak at the end of final response text, including the observed `<turn|>` suffix. The live trace reports which terminator was removed so runtime/template configuration can still be investigated rather than silently hidden.
